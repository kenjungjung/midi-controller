...
# schedule_message の単位は約100ms なので、1 = 約100ms ≈ 10fps。
METER_POLL_INTERVAL = 1
...
class KenjungMIDI(ControlSurface):
    ...
    def _get_raw_level(self, track):
        """
        ミュート・フェーダーを考慮した実効レベルを返す。

        output_meter_left/right を使う。output_meter_level はAbleton内部でピーク
        ホールドされるためリアルタイム性がなく、left/right の方がビート感が出る。
        フェーダー位置 (volume.value) を乗算してポストフェーダーに近づける。
        volume.value は 0dB = 約0.85 なので 0.85 で正規化する。
        """
        if track.mute:
            return 0.0
        fader = min(1.0, track.mixer_device.volume.value / 0.85)
        return max(track.output_meter_left, track.output_meter_right) * fader

    def _poll_meters(self):
        """
        全トラックの出力レベルを読み取ってESP32に送る。
        ...
        tracks = self._get_tracks()
        for i in range(MAX_TRACKS):
            raw = self._get_raw_level(tracks[i]) if i < len(tracks) else 0.0
            self._send_track_meter(i, raw)
    ...
    def _build_track_listeners(self):
        ...
        for track in self._get_tracks()[:MAX_TRACKS]:
            # partial でトラックを束縛する。
            # lambda やネスト関数でループ変数を捕捉するとクロージャ問題が起きるため。
            fn = partial(self._handle_track_color_change, track)
    ...
    def _send_track_color(self, index, color_int):
        ...
        SysEx フォーマット:
          F0  : SysEx 開始 / 7D: MANUFACTURER_ID / 01: SYSEX_TRACK_COLOR
          idx : トラック番号 (0–11) / r g b: 各色 (0–127) / F7: SysEx 終了
        """
        r, g, b = _color_to_7bit_rgb(color_int)
        self._send_midi((0xF0, MANUFACTURER_ID, SYSEX_TRACK_COLOR, index, r, g, b, 0xF7))
    ...
    def _send_track_meter(self, index, level_float):
        ...
        SysEx フォーマット:
          F0  : SysEx 開始 / 7D: MANUFACTURER_ID / 03: SYSEX_TRACK_METER
          idx : トラック番号 (0–11) / lvl: 出力レベル (0–127) / F7: SysEx 終了
        """
        self._send_midi((0xF0, MANUFACTURER_ID, SYSEX_TRACK_METER, index, _meter_to_7bit(level_float), 0xF7))
